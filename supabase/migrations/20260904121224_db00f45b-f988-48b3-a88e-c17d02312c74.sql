...
CREATE OR REPLACE FUNCTION public.get_pending_friend_profiles()
 RETURNS TABLE(id uuid, username text, handle text, avatar_url text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
GRANT EXECUTE ON FUNCTION public.get_pending_friend_profiles() TO authenticated;